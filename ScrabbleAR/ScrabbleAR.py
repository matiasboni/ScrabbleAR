...
>>>>>>> 4854db4fee6233555e9939b4368d204169cb72b5
...
def retornar_tablero(nivel):
    if nivel =='Medio':
        return retornar_tablero_medio()
    elif nivel=='Dificil':
        return retornar_tablero_dificil()
    elif nivel=='Facil':
        return retornar_tablero_facil()
...
def buscar_combinacion(vector_compu,valores_letras):
    ...
    palabra=""
    posiciones=[]
    lista_pattern=list(pattern.es.lexicon.keys())
    for i in lista_pattern:
=======
def buscar_combinacion(vector_compu,valores_letras):
    letras=[]
...
<<<<<<< HEAD
def actualizar_tablero(palabra,orientacion,pos,window):
    if orientacion=="Vertical":
        for i in range(0,len(palabra)):
                window.FindElement((pos[0]+i,pos[1])).Update(text=palabra[i],disabled=True)
    elif orientacion=="Horizontal":
        for i in range(0,len(palabra)):
                window.FindElement((pos[0],pos[1]+i)).Update(text=palabra[i],disabled=True)
    ...
    if pos[0]+len(palabra)<=14:
        ...
        for i in range(0,len(palabra)):
            if tablero_aux[pos[0]+i][pos[1]]!=0:
                ok=False
        if ok:
            actualizar_tablero(palabra,"Vertical",pos,window)
            return True
        else:
            return False
    else:
    ...
    if pos[1]+len(palabra)<=14:
        ...
        for i in range(0,len(palabra)):
            if tablero_aux[pos[0]][pos[1]+i]!=0:
                ok=False
        if ok:
            actualizar_tablero(palabra,"Horizontal",pos,window)
            return True
        else:
            return False
    else:
        ...
        if palabra!="":
            if tablero_aux[7][7]==0:
                ...
                if orientacion=="Vertical":
                    ...
                    pos=(7,random.randint(0,len(palabra)-1))
                actualizar_tablero(palabra,orientacion,pos,window)
                break
            else:
                ok=False
                while not ok:
...
def actualizar_letras_jugador(window, letras_a_cambiar, nuevos, valores_letras, vector_jugador):
    for i in letras_a_cambiar:
        vector_jugador[i]=nuevos[i]
        window.FindElement(i).Update(valores_letras[nuevos[i]]) 
...
def todas_las_pos(evento, coordenadas_palabras, movimiento, cant):
	...
	if movimiento=='horizontal':
		posiciones_validas=horizontal(coordenadas_palabras[0],coordenadas_palabras[len(coordenadas_palabras)-1])
...
def turno_jugador(window,tablero_aux, turno,vector_jugador, cantidad_letras_jugador, valores_letras,contador_partida):
	palabra=[]
	pos_de_letras=(0,1,2,3,4,5,6)
	pos_validas=None
	coordenada_palabras=[]
	letras_usadas=[]
	cant=0
	...
	while True:
		...
		if event in pos_de_letras:
			...
			if evento!=None:
				if cant==1:
					movimiento= forma_de_movimiento(listar[0],evento)
				palabra.append(event)
				tablero_aux[evento[0]][evento[1]]=vector_jugador[event]
				window.FindElement(evento).Update(event)
				coordenada_palabra.append(evento)
				pos_validas=todas_las_pos(evento, coordenada_palabra, movimiento, cant)
				cant=1
				letras_usadas.append(event)
			if event =='Verificar': 
				verificacion=Verificar_palabra(palabra.split(), nivel)
...
def jugar(window,cantidad_de_letras,dic):
	...
	contador_partida=0
	for i in range(0,7):
		window.FindElement("j"+str(i)).Update(valores_letras[vector_jugador[i]])
	turno=random.choice([True,False])
	turno=False
	if turno:
		turno_jugador(window, tablero_aux, turno, vector_jugador, cantidad_letras_jugador, valores_letras,contador_partida)
	else:
		turno_compu(window,tablero_aux,valores_letras,vector_compu,contador_partida,cantidad_letras_compu)
...
def tablero_de_juego(dic):
    ...
    letras_usuario=[[sg.Button("",key="j"+str(a), size=(5,2)) for a in range(7)]]
    ...
    if not event in (None,"Terminar"):
        jugar(window,cantidad_de_letras,dic)
    else:
        window.close()
